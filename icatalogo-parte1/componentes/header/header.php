...
<header class="header">
   ...
   <input type="text" id="pesquisar" name="p" value="<?= isset($_GET["p"]) ? $_GET["p"] : "" ?>" placeholder="Pesquisar" />      
     <button <?= isset($_GET["p"]) && $_GET["p"] != "" ? "onClick='limparFiltro()'" : "" ?>>
        ...
        <h1>Fazer login</h1>
        <form  method="POST" action="/backend/projeto-php/icatalogo-parte1/componentes/header/acoesLogin.php">
    ...
    ?>
      ...
      <form id="form-logout"form method="POST" style= "display: none" action="/backend/projeto-php/icatalogo-parte1/componentes/header/acoesLogin.php">
